...
local function drawCell(cellRectID, usedZoom, cellColor, disabled, colls)
	...
	if showPrice then
		...
		if config.showDetailedPrice then
			local label_x = cellRects[cellRectID][3] - cellPadding - (cellInnerSize * 0.048)
			local label_y = cellRects[cellRectID][4] - cellPadding
			DrawDetailedCostLabels(label_x, label_y, uDefID, priceFontSize*0.8)
		end
	end
                            ...
                            for cellRectID, _ in pairs(cellRects) do
                                if not drawncellRectIDs[cellRectID] then
                                    ...
                                    if unitBuildDefID == cellUnitDefID then
                                        ...
                                        RectRoundProgress(cellRects[cellRectID][1] + cellPadding + iconPadding,
                                            cellRects[cellRectID][2] + cellPadding + iconPadding,
                                            cellRects[cellRectID][3] - cellPadding - iconPadding,
                                            cellRects[cellRectID][4] - cellPadding - iconPadding, cellSize * 0.03,
                                            progress, { 0.08, 0.08, 0.08, 0.6 })
                                    end
                                end
                            end
                        end
...
function widget:Initialize()
	...
	if widgetHandler:IsWidgetKnown("Grid menu v2") then
		widgetHandler:DisableWidget("Grid menu v2")
	end
	units.checkGeothermalFeatures()
	if disableWind then
